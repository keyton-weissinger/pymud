...
class QueryDialog(BasicDialog):
    ...
    def create_buttons(self):
        ...
        cancel_button = EasternButton(
            text="取消", handler=(lambda: self.set_done(False))
        )
    ...
    def create_body(self) -> AnyContainer:
        ...
        body = HSplit(
            [
                ...
                Label(
                    HTML(
                        '<b fg="red">PYMUD {0}</b> - a MUD Client Written in Python'.format(
                ...
                Label(
                    HTML(
                        "作者: <b>{0}</b> <b>E-mail</b>: <u>{1}</u>".format(
                            Settings.__author__, Settings.__email__
                        )
                    ),
                    align=WindowAlign.CENTER,
                ),
                ...
                Label(
                    "系统:{} {}   Python版本:{}".format(
                        platform.system(), platform.version(), platform.python_version()
                    ),
                    align=WindowAlign.CENTER,
                ),
                Window(height=1),
